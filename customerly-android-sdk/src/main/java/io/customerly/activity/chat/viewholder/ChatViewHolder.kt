package io.customerly.activity.chat.viewholder
...
import io.customerly.activity.chat.TYPING_NO_ONE
...
internal sealed class ChatViewHolder (
    ...
    class Typing(recyclerView: RecyclerView): ChatViewHolder(recyclerView = recyclerView, layoutRes = R.layout.io_customerly__li_bubble_account_typing) {
        init {
            ...
            this.icon?.also { icon ->
                icon.visibility = if (isFirstMessageOfSender && typingAccountID != TYPING_NO_ONE) {
                    ClyImageHandler.request(
                            request = ClyImageRequest(context = chatActivity, url = urlImageAccount(accountId = typingAccountID, sizePX = this.iconSize))
                                    .fitCenter()
                                    .transformCircle()
    ...
    internal sealed class Message(
            ...
            ) : ChatViewHolder(recyclerView = recyclerView, layoutRes = layoutRes) {
        ...
        override fun apply(chatActivity: ClyChatActivity, message: ClyMessage?, dateToDisplay: String?, isFirstMessageOfSender: Boolean) {
            ...
            this.icon?.also { icon ->
                icon.visibility = if (isFirstMessageOfSender) {
                    ClyImageHandler.request(
                            request = ClyImageRequest(context = chatActivity, url = message.getImageUrl(sizePx = this.iconSize))
                                    .fitCenter()
                                    .transformCircle()
                                    .resize(width = this.iconSize)
                                    .placeholder(placeholder = R.drawable.io_customerly__ic_default_admin)
                    ...
                    when {
                        attachment.isImage() -> {
                            ...
                            if(path?.isNotEmpty() == true) {
                                ClyImageHandler.request(
                                        request = ClyImageRequest(context = chatActivity, url = path)
                                                .centerCrop()
