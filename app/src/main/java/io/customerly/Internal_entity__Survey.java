 ...
 */
class Internal_entity__Survey implements Parcelable {
    ...
    final int survey_id, limit_from, limit_to, step;
    @SURVEY_TYPE final int type;
    @NonNull final String thankyou_text, title, subtitle;
    final boolean seen;
    @Nullable final Choice[] choices;
    ...
    /************************************ PARCELABLE IMPLEMENTATION *******************************/
    private Internal_entity__Survey(@NonNull Parcel in) {
        this.survey_id = in.readInt();
        this.limit_from = in.readInt();
        this.limit_to = in.readInt();
        this.step = in.readInt();
        int type_from_parcel = in.readInt();
        this.type = type_from_parcel < 0 ? TYPE_END_SURVEY : type_from_parcel > LASTTYPE ? LASTTYPE : type_from_parcel;
        this.thankyou_text = in.readString();
    ...
    @Override public void writeToParcel(@NonNull Parcel dest, int flags) {
        ...
        dest.writeString(this.thankyou_text);
        dest.writeString(this.title);
        dest.writeString(this.subtitle);
        dest.writeByte((byte) (seen ? 1 : 0));
        dest.writeTypedArray(this.choices, flags);
    }
    @Override public int describeContents() { return 0; }
    @NonNull public static final Creator<Internal_entity__Survey> CREATOR = new Creator<Internal_entity__Survey>() {
        @Override public Internal_entity__Survey createFromParcel(@NonNull Parcel in) { return new Internal_entity__Survey(in); }
        @Override public Internal_entity__Survey[] newArray(int size) { return new Internal_entity__Survey[size]; }
    };
}